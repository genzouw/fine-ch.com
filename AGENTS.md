...
本ドキュメントは、自律型コーディングエージェント (Jules / Devin / Codex / Claude Code / GitHub Copilot / Cursor / Cline / Windsurf / Aider / Sweep / PR-Agent 等) が、公開 OSS リポジトリ [`genzouw/fine-ch.com`](https://github.com/genzouw/fine-ch.com) で作業し Pull Request を作成するときに **必ず守るべき規範** を定義します。
...
上記をひとつでも満たさないサービスを組み込む PR は **MUST NOT** で、提出された場合は内容の良し悪しにかかわらずマージがブロックされる、またはリポジトリオーナー (@genzouw) の判断が必要になります。

### 1.1 MUST NOT — これらを含む PR はマージがブロックされます

- **MUST NOT**: LLM プロバイダの API キー / トークンを GitHub Secrets に登録し、CI ワークフロー / GitHub Action から呼び出す構成の追加。
  - 関連する環境変数・設定例 (これらに限らない):
...
- **MUST NOT**: 既に本リポジトリに導入済みのツールと機能が重複する追加 (`.github/workflows/` 配下に加え、pre-commit 設定 / Make ターゲット / `package.json` 等の依存関係定義 / `scripts` / 導入済み GitHub App もリポジトリ全体で必ず事前確認すること)。
...
- **MAY**: 利用上限を超えても課金・自動アップグレード・クレジットカード登録が発生せず (超過時はサービスがブロック・機能制限されるのみ)、API キーの登録も不要な、公開 OSS リポジトリ向け完全無料プランの GitHub Marketplace Action / GitHub App (例: CodeRabbit の OSS 向け完全無料プラン)。
...
本ポリシーから外れる導入を検討したい場合は、PR を作成する **前に** Issue で提案し、リポジトリオーナー (@genzouw) の明示的な承認を **SHOULD** 取得してください。承認のない有料サービス導入 PR はマージがブロックされる、またはリポジトリオーナー (@genzouw) の判断が必要になります。
...
本ポリシーは、本ファイル (`AGENTS.md`) が配置されている当リポジトリ [`genzouw/fine-ch.com`](https://github.com/genzouw/fine-ch.com) にのみ適用されます。
[@genzouw](https://github.com/genzouw) が公開している他の公開リポジトリへ同様の方針を適用したい場合は、各リポジトリに個別に本ファイルを配置してください (現時点で複数リポジトリへ自動配布する仕組みはありません)。
