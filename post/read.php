...
$requestUriIsSp = (preg_match ( '/\A.+\/m\/.+\z/u' , $_SERVER['REQUEST_URI']));
#==================================================
#　リクエスト解析
#==================================================
...
if (!empty($_SERVER['PATH_INFO'])) {
	...
	$bbs = $pairs[1];
	$key = $requestUriIsSp ? $pairs[3] : $pairs[2];
	$page = $requestUriIsSp ? $pairs[4] : $pairs[3];
	if (!empty($page)) {
		if (strstr($page, 'n')) {
			$nofirst = 'true';
			$page = str_replace("n","",$page);
		}
		if (substr($page, 0, 1) == 'l') {
			$ls = substr($page,1);
		}
		elseif (strstr($page, '-')) {
			list($st, $to) = explode('-',$page);
			if (!$st) $st = 1;
		}
		else {
			$st = $page;
...
$urlDirs = "${bbs}" . ($requestUriIsSp ? '/m' : '');
?>
<html>
<head>
<base href="<?php echo ($requestUriIsSp ? '../' : ''); ?>../../../">
...
if(!JIKAN_KISEI or ($JIKAN > JIKAN_END and $JIKAN < JIKAN_START)) {
	echo "<a href=\"../post/read.php/$urlDirs/$key/\">全部</a>\n";
}
for ($iCnt = 1; $iCnt <= $LINENUM; $iCnt += 100){
	$iTo = $iCnt + 99;
	echo "<a href=\"../post/read.php/$urlDirs/$key/$iCnt-$iTo\">$iCnt-</a>\n";
}
...
?>
<a href="../post/read.php/<?=$urlDirs."/".$key?>/l50">最新50</a>
<?=$alert?>
...
while ($s <= $END) {
	...
	if (!JIKAN_KISEI or ($JIKAN > JIKAN_END and $JIKAN < JIKAN_START)) {
		...
		if ($i < 100) {
			...
			$message = str_replace("/post/read.php/$urlDirs/$key/",'',$message);
			$mailto = $mail ? "<a href=\"mailto:$mail\"><b>$name</b></a>" : "<font color=\"$SETTING[BBS_NAME_COLOR]\"><b>$name</b></font>";
			echo "<dt>$s ：$mailto ： $time<br><dd> $message <br><br><br>\n";
			$s++;
			$i++;
...
if ($LINENUM <= 1000) {
	if ($LINENUM >= $s) echo "<center><a href=\"../post/read.php/$urlDirs/$key/$END-\">続きを読む</a></center><hr>\n";
	else echo "<center><a href=\"../post/read.php/$urlDirs/$key/$LINENUM-\">新着レスの表示</a></center><hr>\n";
}
$t = $s + 99;
$u = 1;
echo "<a href=\"../$urlDirs/index.html\">掲示板に戻る</a> <a href=\"../post/read.php/$urlDirs/$key/\">全部</a>\n";
if ($mae) {
	if ($mae == 1) echo "<a href=\"../post/read.php/$urlDirs/$key/1\">前100</a>\n";
	else {
		if ($mae > 100) $u = $mae-99;
		echo "<a href=\"../post/read.php/$urlDirs/$key/$u-$mae\">前100</a>\n";
	}
}
...
function DispError($topic) {
    global $url, $bbs, $key, $requestUriIsSp, $SETTING;
    ...
    echo "td.type1 {color: #ffffff;text-align: left;}a.bigline {color: #000000;text-decoration: none;}";
    echo "--></style>";
    echo "</head>";
    ...
    echo "<dd>$topic<br><br><br>";
    echo "</dl>";
    echo "<hr>";
    echo "<font size=-2>".VERSION."</font>";
    echo "<hr>";
    echo "<p>";
    if (is_file('../$urlDirs/kako/$key.html')) {
        echo "隊長! 過去ログ倉庫で、<a target='_self' href='$url/$urlDirs/kako/$key.html?'>スレッド$key.html</a> を発見しました。";
    } else {
        echo "<a target='_self' href='$url/$urlDirs/kako/'>過去ログ倉庫</a>にもありませんでした。<br>問い合わせても見つかる可能性はほとんどありません。";
    }
    echo "</body></html>;";
    if (GZ_FLAG) {
        ob_end_flush();
